...
export function migrate(x:any):SaveGame|null {if(!x||typeof x!=='object'||!Array.isArray(x.heroes)||!Array.isArray(x.completedNights))return null;if(x.saveVersion===2)return x as SaveGame;if(x.saveVersion===1)return{...x,saveVersion:2,heroes:x.heroes.map(migrateHero),settings:{master:.8,music:.55,sfx:.75,reducedMotion:false,autoPause:true}};return null;}
