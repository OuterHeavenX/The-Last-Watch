...
describe('deterministic defense simulation',()=>{
  ...
  it('downed heroes receive deterministic recoverable injuries',()=>{const heroes=starterHeroes();for(const h of heroes){h.maxHp=1;h.hp=1;h.attack=0;}const b=new Battle(4,heroes,[],9);b.run();expect(heroes.some(h=>h.injury&&h.injury.recoveryDays>=1)).toBe(true);});
});
